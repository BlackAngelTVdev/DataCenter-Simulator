...
func _ready() -> void:
	...
	# 7. Pas de re-déblocage : une seconde passe ne renvoie rien.
	var again := Achievements.check_all()
	print("TEST again=", again.size())
	var again_ok: bool = again.is_empty()

	var ok: bool = early_ok and first_ok and million_ok and ten_ok and ddos_ok and cats_ok and again_ok
